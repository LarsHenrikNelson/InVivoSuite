...
def pl2_ad(filename, channel):
    ...
    for i in range(num_fragments_returned.value):
        tmp_fragment_timestamps.append(
            fragment_timestamps[i] / file_info.m_TimestampFrequency
        )
    values = np.array(values)
    # Fill in and return named tuple.
    return PL2Ad(
        achannel_info.m_SamplesPerSecond,
...
def pl2_events(filename, channel):
    ...
    return PL2DigitalEvents(
        num_events_returned.value,
...
def pl2_info(filename):
    ...
    for i in range(file_info.m_TotalNumberOfSpikeChannels):
        ...
        if schannel_info.m_ChannelEnabled:
            spike_counts.append(
                spike_info(
                    schannel_info.m_Channel,
                    schannel_info.m_Name.decode("ascii"),
    ...
    for i in range(file_info.m_NumberOfDigitalChannels):
        ...
        if echannel_info.m_NumberOfEvents:
            event_counts.append(
                event_info(
                    echannel_info.m_Channel,
                    echannel_info.m_Name.decode("ascii"),
    ...
    for i in range(file_info.m_TotalNumberOfAnalogChannels):
        ...
        if achannel_info.m_ChannelEnabled:
            ad_counts.append(
                ad_info(
                    achannel_info.m_Channel,
                    achannel_info.m_Name.decode("ascii"),
