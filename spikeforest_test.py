...
import os
import spikeforest_test as sf
from spikeforest.load_extractors.MdaRecordingExtractorV2.MdaRecordingExtractorV2 import (
    readmda,
)
...
def main():
    ...
    for R in all_recordings:
        ...
        obj["self_reference"] = ka.store_json(
            obj, label="{}/{}/{}.json".format(studyset_name, study_name, recname)
        )
        ...
        obj2["self_reference"] = ka.store_json(
            obj2,
            label="{}/{}/{}.firings_true.json".format(
                studyset_name, study_name, recname
            ),
        )
        with open(firings_true_file, "w") as f:
            json.dump(obj2, f, indent=4)
        study = {}
        study["self_reference"] = ka.store_json(
            study, label="{}.json".format(study_name)
        )
        with open(os.path.join(studydir_local, study_name + ".json"), "w") as f:
            json.dump(study, f, indent=4)
        studyset = {}
        studyset["self_reference"] = ka.store_json(
            studyset, label="{}.json".format(studyset_name)
        )
    ...
    studysets_obj = dict(StudySets=studySets)
